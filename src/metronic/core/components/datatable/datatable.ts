        ...
        if (!response.ok) {
          if (response.status === 404 || response.status === 500) {
            ...
            this._fireEvent("error", {
              error: new Error("API endpoint not found"),
            });
          ...
          this._fireEvent("error", {
            error: new Error(
              "Error performing fetch request: " + response.statusText
            ),
          });
          this._dispatchEvent("error", {
            error: new Error(
    ...
    const th =
      typeof sortField === "number"
        ...
        : (this._theadElement.querySelector(
            `th[data-datatable-column="${String(
              sortField
            )}"], th[data-datatable-column-sort="${String(sortField)}"]`
          ) as HTMLElement);
