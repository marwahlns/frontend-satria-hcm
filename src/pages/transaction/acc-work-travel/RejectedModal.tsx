...
const RejectedModal = ({
  ...
  const schema = yup.object().shape({
    rejected_remark: yup.string().required("Please fill out accepted remark"),
  });
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const result = await Swal.fire({
        title: `Are you sure?`,
        text: `Do you want to reject this work travel request?`,
        icon: "warning",
      ...
      const response = await axios.put(
        `${process.env.NEXT_PUBLIC_API_URL}/api/trx/trxOfficialTravel/rejected/${selectedData.id}`,
        {
          ...data,
      ...
      if (response.status == 201) {
        Swal.fire({
          title: "Success!",
          text: "Work travel has been successfully rejected",
          icon: "success",
      ...
      Swal.fire({
        title: "Error!",
        text: "Failed to accept work travel. Please try again.",
        icon: "error",
        confirmButtonText: "OK",
      });
    }
