  ...
  const onSubmit = async (data) => {
    try {
      ...
      const result = await Swal.fire({
        title: `Are you sure?`,
        text: `Do you want to reject this resign request?`,
        icon: "warning",
      ...
      const response = await axios.put(
        `${process.env.NEXT_PUBLIC_API_URL}/api/trx/trxResign/rejected/${selectedData.id}`,
        {
          ...data,
      ...
      if (response.status == 201) {
        Swal.fire({
          title: "Success!",
          text: "Resign has been successfully rejected",
          icon: "success",
