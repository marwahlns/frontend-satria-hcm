  ...
  const onSubmit = async (data) => {
    try {
      ...
      const result = await Swal.fire({
        title: `Are you sure?`,
        text: `Do you want to accept this mutation request?`,
        icon: "warning",
      ...
      const response = await axios.put(
        `${process.env.NEXT_PUBLIC_API_URL}/api/trx/trxOvertime/accepted/${selectedData.id}`,
        {
          ...data,
      ...
      if (response.status == 201) {
        Swal.fire({
          title: "Success!",
          text: "Overtime has been successfully accepted",
          icon: "success",
      ...
      Swal.fire({
        title: "Error!",
        text: "Failed to accept overtime. Please try again.",
        icon: "error",
        confirmButtonText: "OK",
      });
    }
