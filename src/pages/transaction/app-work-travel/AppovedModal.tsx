...
const ApprovedModal = ({
  ...
  const schema = yup.object().shape({
    approved_remark: yup.string().required("Please fill out Approved remark"),
  });
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const result = await Swal.fire({
        title: `Are you sure?`,
        text: `Do you want to approve this work travel request?`,
        icon: "warning",
      ...
      const response = await axios.put(
        `${process.env.NEXT_PUBLIC_API_URL}/api/trx/trxOfficialTravel/approved/${selectedData.id}`,
        {
          ...data,
      ...
      if (response.status == 201) {
        Swal.fire({
          title: "Success!",
          text: "Work travel has been successfully Approved",
          icon: "success",
      ...
      Swal.fire({
        title: "Error!",
        text: "Failed to approve work travel. Please try again.",
        icon: "error",
        confirmButtonText: "OK",
      });
    }
    ...
    <Modal isModalOpen={isModalOpen}>
      <div className="flex justify-between items-center border-b pb-4">
        <h3 className="text-lg font-semibold text-gray-900">
          Submission Official Travel Data
        </h3>
            ...
            <div className="mt-6">
              ...
              {errors.approved_remark && (
                <p className="text-red-500 text-sm mt-1">
                  {errors.approved_remark.message}
                </p>
              )}
            </div>
          </div>
