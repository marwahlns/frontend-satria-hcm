...
export default function Home() {
  ...
  const areaOptions: ApexOptions = {
    ...
    tooltip: {
      custom: function ({ series, seriesIndex, dataPointIndex, w }) {
        ...
        const formattedDate = `${dayName}, ${day} ${monthNames[Number(month) - 1]} ${year}`;
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await
          axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/trx/tren-submission`, {
  ...
  useEffect(() => {
    const fetchAttendance = async () => {
      ...
      try {
        const token = Cookies.get("token");
        const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/trx/tren-attendance`, {
        ...
        const days = data.map((item: any) => Number(item.tanggal.split("-")[2]));
