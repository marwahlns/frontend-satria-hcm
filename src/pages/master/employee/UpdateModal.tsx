...
const UpdateModal = ({
  ...
  const schema = yup.object().shape({
    ...
    bdate: yup.string().required("Birth date is required."),
    gender: yup
      .object({
  ...
  useEffect(() => {
    ...
    const fetchAndSet = async () => {
      const maritalStatus = selectedData?.user_detail?.marital_status
        ? await getSelectedMaritalStatus(
            selectedData.user_detail.marital_status
          )
        : null;
      ...
      reset({
        ...
        bdate: selectedData.user_detail?.birth_date
          ? new Date(selectedData.user_detail.birth_date)
              .toISOString()
              .split("T")[0]
          : "",
        gender:
          genderOptions.find(
            (opt) => opt.value === selectedData.user_detail?.gender
          ) ?? null,
        marital_status: maritalStatus,
        address: selectedData.user_detail?.address,
        vendor: vendor,
        join_date: selectedData.user_detail?.join_date
          ? new Date(selectedData.user_detail.join_date)
              .toISOString()
              .split("T")[0]
          : "",
        end_date: selectedData.user_detail?.end_date
          ? new Date(selectedData.user_detail.end_date)
              .toISOString()
              .split("T")[0]
          : "",
        plant: plant,
  ...
  const maritalOptions = async (inputValue) => {
    try {
      ...
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/api/master/user/getMarital`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const vendorOptions = async (inputValue) => {
    try {
      const token = Cookies.get("token");
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/api/master/user/getVendor`,
        {
    ...
    try {
      ...
      if (response.data.success) {
        return response.data.data.data.map((worklocation) => ({
          ...
          label:
            worklocation.worklocation_code +
            " | " +
  ...
  const plantOptions = async (inputValue) => {
    try {
      const token = Cookies.get("token");
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/api/master/user/getPlant`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const klasifikasiOptions = async (inputValue) => {
    try {
      const token = Cookies.get("token");
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/api/master/user/getKlasifikasi`,
        {
          headers: { Authorization: `Bearer ${token}` },
          params: { search: inputValue },
        }
  ...
  const getSelectedSuperior = async (selectedId) => {
    ...
    try {
      ...
      if (response.data.success) {
        const found = response.data.data.data.find(
          (sup) => sup.personal_number === selectedId
        );
        return found
          ? {
              ...
              <Controller
                ...
                render={({ field }) => (
                  <input
                    {...field}
                    type="text"
                    className={clsx("input", errors.nrp && "border-red-500")}
                  />
                )}
              />
              {errors.nrp && (
              ...
              <Controller
                ...
                render={({ field }) => (
                  <input
                    {...field}
                    type="date"
                    className={clsx("input", errors.bdate && "border-red-500")}
                  />
                )}
              />
              {errors.bdate && (
                ...
                render={({ field }) => (
                  <AsyncSelect
                    ...
                    styles={{
                      control: (base, state) => ({
                        ...base,
                        borderColor: errors.marital_status
                          ? "#EF4444"
                          : "#DBDFE9",
                        "&:hover": {
                          borderColor: state.isFocused ? "#DBDFE9" : "#EF4444",
                ...
                render={({ field }) => (
                  <AsyncSelect
                    ...
                    styles={{
                      control: (base, state) => ({
                        ...base,
                        borderColor: errors.worklocation
                          ? "#EF4444"
                          : "#DBDFE9",
                        "&:hover": {
                          borderColor: state.isFocused ? "#DBDFE9" : "#EF4444",
